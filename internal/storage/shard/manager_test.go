...
func TestShardManager_GetShard(t *testing.T) {
	m := NewShardManager(t.TempDir(), time.Hour, memtable.DefaultMemTableConfig(), nil, newTestMgr(t, t.TempDir()))
...
func TestShardManager_PersistAll(t *testing.T) {
	tmpDir := t.TempDir()
	m := NewShardManager(tmpDir, time.Hour, memtable.DefaultMemTableConfig(), nil, newTestMgr(t, tmpDir))
...
func TestRetentionService_Cleanup(t *testing.T) {
	tmpDir := t.TempDir()
	m := NewShardManager(tmpDir, 500*time.Millisecond, memtable.DefaultMemTableConfig(), nil, newTestMgr(t, tmpDir))
