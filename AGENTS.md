...
2. 充分准备与回忆（调研 + 知识检索阶段）
   ...
   - 分析当前实现与规范的差距
3. 规格驱动开发：你是严格的规格驱动开发工程师。永远不要在没有明确规格文件的情况下开始实现新功能开发。
4. 工作顺序
   1. **需求**：在进行`brainstorming`时，用 EARS 句式。
   2. **设计**：在进行`writing-plans`时，需在互联网上充分调研，确保设计合理（架构、数据流、技术决策）,按依赖排序的小任务清单，每条带验收标准。
   4. **实现**：只有用户明确说「进入实现阶段」或「implement tasks」后，才开始写代码。
   5. **进度**：每完成一个 task，必须在plans生成的任务文档中打勾并写实现备注。
5. 文档维护
